...
For `fred-public-data`, security-sensitive reports should be handled privately by the repository owner.

When reporting an issue, redact any local environment details that are not required to understand the problem.

## Scope

- Do not commit credentials, tokens, API keys, private keys, or machine-specific local configuration.
- The `FRED_API_KEY` environment variable must never be committed or logged. Use a `.env` file excluded by `.gitignore` or a secrets manager such as `auto-pass`.
